...
#         # Forward propagation
#         a1_enc = x_train_enc.transpose()  # (784,64)
#         z2_enc = TruncPrMul(np.dot(W2_enc, a1_enc)) + b2_enc  # (128,784) * (784,64)
#         a2_enc = relu_enc(z2_enc)  # (128,64)
#         z3_enc = TruncPrMul(np.dot(W3_enc, a2_enc)) + b3_enc  # (10,128) * (128,64)
#         a3_enc = relu_enc(z3_enc)  # (10,64)
#
#         # Backward propagation
#         delta_3_enc = (a3_enc - y_train_enc.transpose())
#         delta_2_enc = drelu_enc(z2_enc) * TruncPrMul(np.dot(W3_enc.transpose(), delta_3_enc))
#
#         gradient_b3_enc = delta_3_enc
#         gradient_b2_enc = delta_2_enc
#         gradient_W3_enc = TruncPrMul(np.dot(delta_3_enc, a2_enc.transpose()))
...
#         sum_gradient_W2_enc = gradient_W2_enc
#
#         b3_enc -= LR * div_enc(decode(sum_gradient_b3_enc), BATCH_SIZE)
#         b2_enc -= LR * div_enc(decode(sum_gradient_b2_enc), BATCH_SIZE)
#         W3_enc -= LR * div_enc(decode(sum_gradient_W3_enc), BATCH_SIZE)
#         W2_enc -= LR * div_enc(decode(sum_gradient_W2_enc), BATCH_SIZE)
#
#         # Training Error
#         sum_of_training_error += np.sum(np.argmax(decode(a3_enc), axis=0) != np.argmax(y_train.transpose(), axis=0)).astype(int)
#
#     test_layer_2_enc = relu_enc(TruncPrMul(np.dot(W2_enc, encode(x_test_total).transpose())) + b2_enc)
#     test_layer_3_enc = relu_enc(TruncPrMul(np.dot(W3_enc, test_layer_2_enc)) + b3_enc)
#     sum_of_test_error = np.sum(np.argmax(decode(test_layer_3_enc), axis=0) == np.argmax(y_test_total.transpose(), axis=0)).astype(
#         int)
#     print("epoch:", epoch, "training_error:", sum_of_training_error, "accuracy_rate:", sum_of_test_error / TEST_SIZE)
